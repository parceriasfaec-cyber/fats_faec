...
def _peso_numero(valor):
    """O peso é guardado como texto (ex: '320,5'). Vira número quando der."""
...
def _montar_excel_animais_alocados(linhas, meta):
    """Monta a planilha de animais alocados. `linhas` vem de um LEFT JOIN
    produtores x animais (produtor sem animal vem com brinco_faec vazio)."""
    wb = Workbook()
    header_fill = PatternFill(start_color="1F4E2C", end_color="1F4E2C", fill_type="solid")
    header_font = Font(color="FFFFFF", bold=True)
...
@app.route("/exportar-animais-alocados")
def exportar_animais_alocados():
    ...
    buf = _montar_excel_animais_alocados(
        [dict(r) for r in rows], LIMITE_ANIMAIS_POR_PRODUTOR
    )
    return send_file(
        buf,
        mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
